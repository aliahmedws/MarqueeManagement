...
import { ListService, LocalizationModule, PagedResultDto } from '@abp/ng.core';
import { MarqueeService, MarqueeDto, GetMarqueeListDto, CreateMarqueeDto, UpdateMarqueeDto } from '../proxy/marquees';
...
@Component({
  ...
  imports: [CommonModule, CardModule, NgbDatepickerModule, ModalComponent,FormsModule, CommonModule,ThemeSharedModule, ReactiveFormsModule, NgbDatepickerModule, 
    CardModule, ModalComponent,PageModule,NgbDropdownModule,LocalizationModule,NgxDatatableModule],
...
export class Marquees implements OnInit {
  ...
  ngOnInit(): void {
    const marqueeStreamCreator = query => 
      this.marqueeService.getList({ ...query, ...this.filters });
